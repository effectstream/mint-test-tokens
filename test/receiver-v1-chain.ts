...
import * as ShieldedIssuer from "../contracts/v1/managed/shielded/contract/index.js";
import * as UnshieldedIssuer from "../contracts/v1/managed/unshielded/contract/index.js";
...
const shieldedPath = resolve(root, "contracts/v1/managed/shielded");
const unshieldedPath = resolve(root, "contracts/v1/managed/unshielded");
...
try {
  const provider = (zkConfigPath: string, label: string) =>
    initializeMidnightProviders(signerWallet, env, {
      privateStateStoreName: resolve(root, `.local/receiver-chain-${runId}-${label}`),
      zkConfigPath,
    });
